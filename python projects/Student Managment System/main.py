...
def addstudent():
    ...
    def submitadd():
        ...
        try:
            ...
            con.commit()
            res = messagebox.askyesnocancel('Notifications', 'Id {} Name {} Added sucessfully\nDo you want to clean the form?'.format(id,name),parent=addroot)
            if(res==True):
                idvar.set('')
                namevar.set('')
                mobilevar.set('')
            ...
            messagebox.showerror('Invalid ID or database', 'ID already exist try another ID\nMay be you are not connected to database.', parent=addroot)
    ...
    addroot.iconbitmap('D:\\PycharmProjects\\python  Projects\\Student Managment System\\icon.ico')
...
def searchstudent():
    ...
    searchroot.iconbitmap('D:\\PycharmProjects\\python  Projects\\Student Managment System\\icon.ico')
...
def updatestudent():
    ...
    updateroot.iconbitmap('D:\\PycharmProjects\\python  Projects\\Student Managment System\\icon.ico')
...
def Connectdb():
    ...
    def submitdb():
        ...
        try:
            ...
            mycursor.execute(strr)
            strr = 'CREATE TABLE studentdata(id INT(11) NOT NULL PRIMARY KEY, name VARCHAR(25), mobile VARCHAR(10), email VARCHAR(30),address VARCHAR(120), gender VARCHAR(6), dob VARCHAR(20), date VARCHAR(20), time VARCHAR(20));'
            mycursor.execute(strr)
    ...
    dbroot = Toplevel()
    dbroot.iconbitmap('D:\\PycharmProjects\\python  Projects\\Student Managment System\\icon.ico')
    dbroot.grab_set()
    dbroot.title('Connect to DataBase - Kavyansh')
...
"""
root = Tk()
root.title('StudentManagementSystem - Kavyansh')
root.configure(bg='gold2')
root.geometry('1000x600+183+50')
root.iconbitmap('D:\\PycharmProjects\\python  Projects\\Student Managment System\\icon.ico')
...
#Show All
allbtn = Button(DateEntryFrame, text='4. All Students', width=19, font='Helvatica 15 bold', borderwidth=6, bg='purple2', relief=RIDGE, command=allstudent)
allbtn.pack(side=TOP, expand=True)

#Export Data
exportbtn = Button(DateEntryFrame, text='5. Export Data', width=19, font='Helvatica 15 bold', borderwidth=6, bg='purple2', relief=RIDGE, command=exportdata)
exportbtn.pack(side=TOP, expand=True)

#Student Exit
exitbtn = Button(DateEntryFrame, text='6. Exit', width=19, font='Helvatica 15 bold', borderwidth=6, bg='purple2', relief=RIDGE, command=exitstudent)
exitbtn.pack(side=TOP, expand=True)

mylable = Label(DateEntryFrame, text='Devoloped By - Kavyansh', font='Helvatica 18 bold', borderwidth=6, bg='red', relief=RIDGE)
